...
struct TodayView: View {
    ...
    /// the resolved diameter so the gauge + its line width scale together.
    @ViewBuilder
        ...
        NoopCard(padding: 12, tint: domain.color) {
            VStack(spacing: 8) {
                ...
                GeometryReader { geo in
                    // `dia` sizes the ring to the cell; the ring is intrinsically dia×dia, so just
                    // CENTER it — don't clamp the ZStack to dia (that would wrap the calibrating/no-data
                    // overlay text, which is wider than the ring).
